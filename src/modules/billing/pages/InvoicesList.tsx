...
export default function InvoicesList() {
  ...
  const resetForm = () => {
    ...
    setStatus("pending"); 
    setDueDate(""); 
    setItems([{ serviceName: "", price: 0, quantity: 1 }]);
    setCreating(false); 
    setEditing(null);
  };
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader 
        title="Invoices" 
        description="Manage your billing and invoicing workflow." 
      />
      ...
      {showForm && (
        <div className="mb-6 rounded-xl border border-border bg-card p-6 shadow-sm space-y-4">
                ...
                <div key={idx} className="flex items-center gap-2">
                  <select 
                    ...
                    onChange={e => { 
                      ...
                      updateItem(idx, "serviceName", e.target.value); 
                      if (svc) updateItem(idx, "price", svc.price); 
                    }} 
          ...
          <div className="flex gap-3 pt-2">
            <Button className="gradient-primary text-primary-foreground border-0" onClick={handleSave}>{editing ? "Update Invoice" : "Create Invoice"}</Button>
              ...
              >
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-2">
                    <Button variant="ghost" size="icon" onClick={() => handleEdit(inv)} className="h-8 w-8 text-primary hover:bg-primary/10">
                      <Pencil className="h-4 w-4" />
                    </Button>
                    <Button variant="ghost" size="icon" onClick={() => handleDelete(inv.id)} className="h-8 w-8 text-destructive hover:bg-destructive/10">
