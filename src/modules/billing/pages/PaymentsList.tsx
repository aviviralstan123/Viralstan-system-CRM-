      ...
      {showForm && (
        <div className="mb-6 rounded-xl border border-border bg-card p-6 shadow-sm space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            <Button className="gradient-primary text-primary-foreground border-0" onClick={handleSave}>{editing ? "Update Record" : "Save Record"}</Button>
              ...
              <tr key={payment.id} className="border-b border-border last:border-0 hover:bg-muted/20 transition-colors">
                ...
                <td className="px-4 py-3">
                  <span className="text-sm text-muted-foreground flex items-center gap-1.5 capitalize">
                    {methodIcons[payment.method]} {payment.method.replace("_", " ")}
                  </span>
                </td>
                <td className="px-4 py-3"><StatusBadge label={payment.status} variant={getInvoiceStatusVariant(payment.status)} /></td>
                <td className="px-4 py-3">
                  <div className="flex items-center gap-2">
                    <Button variant="ghost" size="icon" onClick={() => handleEdit(payment)} className="h-8 w-8 text-primary hover:bg-primary/10">
                      <Pencil className="h-4 w-4" />
                    </Button>
                    <Button variant="ghost" size="icon" onClick={() => handleDelete(payment.id)} className="h-8 w-8 text-destructive hover:bg-destructive/10">
